...
export default function TimetableBuilder({
  ...
  function build(baseStartMinutes, dwell) {
    ...
    for (let i = 0; i < stopsOrdered.length; i++) {
      if (i === 0) {
        out.push({ idx: i, name: names[i], arrival: "", departure: hhmmFromMinutes(dep) });
        continue;
      }
      ...
      if (isLast) {
        out.push({
          ...
          departure: hhmmFromMinutes(arr), // backend needs departureTime
        });
        dep = arr;
      } else {
  ...
  useEffect(() => {
    ...
    const generatedStops = stopsOrdered.map((s, idx) => {
      ...
      if (idx === stopsOrdered.length - 1) departure = departure || arrival;
    ...
    <div style={{ display: "grid", gap: 12, marginTop: 10 }}>
      <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <label>
            Start Time at A
            <input type="time" value={startHHMM} onChange={(e) => setStartHHMM(e.target.value)} style={{ width: "100%", padding: 8 }} />
          </label>

          <label>
            Dwell/Stop Minutes
            <input type="number" min={0} value={dwellMin} onChange={(e) => setDwellMin(e.target.value)} style={{ width: "100%", padding: 8 }} />
          </label>
        </div>
      ...
      <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 10 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.6fr 0.6fr", gap: 8, fontWeight: 900, opacity: 0.8 }}>
          <div>Station</div>
          <div>Arrival</div>
