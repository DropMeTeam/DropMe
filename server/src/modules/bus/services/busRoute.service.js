...
function validatePoint(p, name) {
  ...
  if (!p.label || typeof p.label !== "string") {
    throw new Error(`${name}.label is required`);
  }
...
export async function createBusRoute(payload) {
  ...
  if (!routeNumber || typeof routeNumber !== "string") {
    throw new Error("routeNumber is required");
  }

  if (!["NORMAL", "EXPRESS"].includes(routeType)) {
    throw new Error("routeType must be NORMAL or EXPRESS");
  }
  ...
  if (stops.length > maxStops) {
    throw new Error(`${routeType} routes cannot exceed ${maxStops} stops`);
  }
  ...
  if (Number.isNaN(normalizedDistanceKm) || normalizedDistanceKm < 0) {
    throw new Error("distanceKm must be a valid non-negative number");
  }
  ...
  const doc = await BusRoute.create({
    ...
    routeType,
    start: {
      label: start.label.trim(),
      lat: start.lat,
      lng: start.lng
    },
    end: {
      label: end.label.trim(),
      lat: end.lat,
      lng: end.lng
    },
...
export async function listBusRoutes({ routeType } = {}) {
  ...
  if (routeType && ["NORMAL", "EXPRESS"].includes(routeType)) {
