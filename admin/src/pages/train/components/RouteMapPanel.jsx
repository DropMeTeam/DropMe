...
function FitBounds({ points }) {
  ...
  useEffect(() => {
    ...
    if (points.length === 1) {
      ...
      return; // ✅ do NOT return map.setView(...)
    }
  ...
  <FitBounds points={fitPoints} />
        {markers.map((m) => (
          <Marker key={m.id} position={[m.lat, m.lng]}>
            <Popup>
              <div style={{ fontWeight: 800 }}>{m.idx + 1}. {m.name}</div>
            </Popup>
          </Marker>
        ))}
        {polyline?.length >= 2 ? <Polyline positions={polyline} /> : points.length >= 2 ? <Polyline positions={points} /> : null}
      </MapContainer>
    </div>
  );
}
