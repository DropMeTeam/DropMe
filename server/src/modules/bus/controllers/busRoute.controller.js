...
export async function updateRoute(req, res, next) {
  try {
    // NEW: include distanceKm in allowed patch fields
    const allowed = ["routeNumber", "routeType", "start", "end", "stops", "distanceKm"];
    const patch = {};
    ...
    if (Array.isArray(patch.stops)) {
      patch.stops = patch.stops.map((s, idx) => ({
        ...s,
        order: idx + 1
      }));
    }
    ...
    if (patch.distanceKm !== undefined) {
      patch.distanceKm = Number(patch.distanceKm);
    }

    const updated = await BusRoute.findByIdAndUpdate(req.params.id, patch, {
      new: true,
      runValidators: true
    });
